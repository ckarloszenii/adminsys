  ...
  template: `
    <div class="k-form">
      <label class="k-form-field">
        <span>Desde</span>
        <kendo-datepicker (valueChange)="onStartChange($event)" [format]="format"
                          [(ngModel)]="start" [max]="max" [popupSettings]="popupSettings">
        </kendo-datepicker>
      </label>
      <label class="k-form-field">
        <span>Hasta</span>
        <kendo-datepicker (valueChange)="onEndChange($event)" [format]="format"
...
export class DateRangeFilterComponent implements OnInit, OnDestroy {
  ...
  public get format(): any {
    return localStorage.getItem('formatKendo');
  }
